...
func (p *Provisioner) generateServer(name string, options *OpenstackOptions, pool *unikornv1.ComputeClusterWorkloadPoolSpec, securityGroups securityGroupSet) (*regionapi.ServerWrite, error) {
...
func (p *Provisioner) generateServerCreateSet(options *OpenstackOptions, securityGroups securityGroupSet) (serverCreateSet, error) {
	out := serverCreateSet{}
	...
	for i := range p.cluster.Spec.WorkloadPools.Pools {
		...
		for index := range pool.Replicas {
			...
			request, err := p.generateServer(name, options, pool, securityGroups)
			if err != nil {
				return nil, err
			}
...
func (p *Provisioner) reconcileServers(ctx context.Context, client regionapi.ClientWithResponsesInterface, servers serverPoolSet, securitygroups securityGroupSet, options *OpenstackOptions) error {
	log := log.FromContext(ctx)

	required, err := p.generateServerCreateSet(options, securitygroups)
	if err != nil {
		return err
	}
...
func (p *Provisioner) updateServerStatus(server *regionapi.ServerRead) {
	poolName := getWorkloadPoolTag(server.Metadata.Tags)
