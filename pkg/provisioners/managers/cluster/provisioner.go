...
// OpenstackOptions are acquired from the region controller at
...
// identityOptions collates a set of credentials and options from the identity and
// network to pass to the resource provisioners.
func (p *Provisioner) identityOptions(ctx context.Context, client regionapi.ClientWithResponsesInterface) (*OpenstackOptions, error) {
...
func (p *Provisioner) updateStatus(servers serverPoolSet, options *OpenstackOptions) {
	p.cluster.Status = unikornv1.ComputeClusterStatus{
		SSHPrivateKey: options.SSHPrivateKey,
	}

	for i := range servers {
		p.updateServerStatus(servers[i])
	}
...
func (p *Provisioner) provision(ctx context.Context) error {
	...
	servers, err := p.newServerSet(clientContext, client)
	if err != nil {
		return err
	}
	...
	defer p.updateStatus(servers, options)

	securityGroups, err := p.newSecurityGroupSet(clientContext, client)
	if err != nil {
		return err
	}
	...
	if err := p.reconcileSecurityGroups(clientContext, client, securityGroups); err != nil {
		return err
	}

	if err := p.reconcileServers(clientContext, client, servers, securityGroups, options); err != nil {
		return err
	}
