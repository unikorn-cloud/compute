...
// NOTE: we have sanity checked the input at read time, so it shouldn't crash.
// If it does, you know why...
func getWorkloadPoolTag(tags *coreapi.TagList) string {
	isWorkloadPoolTag := func(tag coreapi.Tag) bool {
		return tag.Name == WorkloadPoolLabel
	}

	return (*tags)[slices.IndexFunc(*tags, isWorkloadPoolTag)].Value
}
